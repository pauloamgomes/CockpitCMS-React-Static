...
function getImageStyleUrl(style, image) {
  if (image && image.styles !== undefined) {
    ...
    if (img && img.path) {
      if (Config.COCKPIT_LOCAL_IMAGES) {
        img.path = img.path.replace(Config.COCKPIT_HOST, "/" + Config.WEBSITE_BASE_PATH);
      }
  ...
  if (image && image.path && !image.path.startsWith(Config.COCKPIT_UPLOADS_DIR)) {
    if (Config.COCKPIT_LOCAL_IMAGES) {
      return ("/" + Config.WEBSITE_BASE_PATH + "/" + Config.COCKPIT_UPLOADS_DIR + image.path).replace("//", "/");
    }
...
//# sourceMappingURL=static.6519e3d8.js.map